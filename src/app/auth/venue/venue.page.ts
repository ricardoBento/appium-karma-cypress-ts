...
export class VenuePage implements OnInit {
  ...
  constructor(
    ...
    this.venueForm = formBuilder.group({
      ...
      region: ['', Validators.compose([Validators.minLength(1), Validators.required])],
      post_code: ['', Validators.compose([Validators.minLength(1), Validators.required])],
    });
  }
  ngOnInit() {
  }
  ...
  onSubmit() {
    if (this.venueForm.valid) {
      this.auth.registerVenue(this.venueForm.value)
        .subscribe(
          (response: Region) => {
            if (response != null) {
