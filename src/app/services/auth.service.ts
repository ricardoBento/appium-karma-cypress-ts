...
export class AuthService {
  ...
  logout() {
    if (this.storage.get('token')) {
      this.http.get(environment.api_url + environment.logout).subscribe((data) => {
  ...
  appLogin(loginForm): Observable<any> {
    ...
    return this.http.post(environment.api_url + environment.login, body, { headers: this.headers.jsonHeader() });
  }
  register(onRegisterForm): Observable<any> {
    ...
    return this.http.post(environment.api_url + environment.register, body, { headers: this.headers.jsonHeader() });
  }
  passReset(passResetFrom): Observable<any> {
    const body: any = {
      email: passResetFrom.value.email,
    };
    return this.http.post(environment.api_url + environment.pass_reset_url, JSON.stringify(body), { headers: this.headers.jsonHeader() });
  }
  loadRegions() {
    return this.http.get(environment.api_url + environment.regions_url, { headers: this.headers.jsonHeader() })
      .subscribe(
        (regions: any) => {
          if (regions) {
            ...
            let error_message = ['No data from loadREgions()'];
  ...
  registerVenue(venueForm: Region) {
    ...
    return this.http.post(environment.api_url + environment.user_venue_url, JSON.stringify(body), { headers: this.headers.jsonHeader() });
  }
  handleError(error) {
